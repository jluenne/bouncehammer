# $Id: 502_bin-tablectl.t,v 1.18.2.1 2013/08/30 23:05:12 ak Exp $
...
SKIP: {
    ...
    my $Test = new Kanadzuchi::Test::CLI(
            'command' => -x './dist/bin/tablectl' ? './dist/bin/tablectl' : './src/bin/tablectl.PL',
    ...
    EACH_TABLE: foreach my $tablename ( keys %$Mtab ) {
        my $keyname = lc $tablename; 
           $keyname =~ s{s\z}{};
        my $tabchar = lc( substr( $tablename, 0, 1 ) ); 
           $tabchar = 'w' if $tabchar eq 'r';
        my $command = q();
        my $xstatus = 0;
        my $xresult = q();
        my $thisidn = 0;
        ...
        WRITE: {
            ...
            INSERT: foreach my $y ( @$Yaml ) {
                ...
                $command = $Test->perl.$Test->command.$Opts.' --table '.$tabchar.q{ --insert --name '}.$y->{ $keyname }.q(');
                $xstatus = scalar(IPC::Cmd::run( 'command' => $command ));

                ok( $xstatus, $tablename.' INSERT: name = '.$y->{ $keyname } );
                ...
                UPDATE1: {
                    # Rewirte Description
                    $command = $Test->perl.$Test->command.$Opts.' --table '.$tabchar.q{ --update --description 'TEST' --id }.$thisidn;
                    $xstatus = scalar(IPC::Cmd::run( 'command' => $command ));
                ...
                UPDATE2: {
                    # To disabled
                    $command = $Test->perl.$Test->command.$Opts.' --table '.$tabchar.' --update --disabled 1 --id '.$thisidn;
                    $xstatus = scalar(IPC::Cmd::run( 'command' => $command ));
                ...
                UPDATE3: {
                    # To enabled
                    $command = $Test->perl.$Test->command.$Opts.' --table '.$tabchar.' --update --disabled 0 --id '.$thisidn;
                    $xstatus = scalar(IPC::Cmd::run( 'command' => $command ));
                ...
                {
                    # DELETE FROM ...
                    $command = $Test->perl.$Test->command.$Opts.' --table '.$tabchar.' --remove --id '.$thisidn;
                    $xstatus = scalar(IPC::Cmd::run( 'command' => $command ));
