# $Id: 024_statistics-stored-bddr.t,v 1.7.2.3 2013/08/30 23:05:12 ak Exp $
...
SKIP: {
    ...
    AGGREAGATE: {
        ...
        isa_ok( $Stat, $T->class );
        isa_ok( $Stat->handle, 'DBI::db' );
        isa_ok( $Stat->cache, 'ARRAY' );

        foreach my $c ( @{ $Btab->fields->{'join'} }, 'hostgroup', 'reason' ) {

            $Aggr = $Stat->congregat( $c );

            isa_ok( $Aggr, 'ARRAY', '->congregat '.$c );
            ...
            isa_ok( $Aggr, 'ARRAY', '->congregat '.$c.' + WHERE Cond.' );
        ...
        foreach my $c ( @{ $Btab->fields->{'join'} }, 'hostgroup', 'reason' ) {
            ...
            isa_ok( $Aggr, 'ARRAY', 'aggregate '.$c.' + WHERE Cond.' );
            foreach my $e ( @$Aggr ) {
