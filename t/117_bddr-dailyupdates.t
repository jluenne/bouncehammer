# $Id: 117_bddr-dailyupdates.t,v 1.2.2.2 2013/08/30 23:05:12 ak Exp $
...
SKIP: {
    ...
    TABLE_CLASS: {
        $Methods = [ 
            'new', 'is_validid', 'is_validcolumn', 'size', 'count',
            'search', 'insert', 'update', 'disable', 'remove'
        ];
        $Class = 'Kanadzuchi::BdDR::DailyUpdates::Table';
        ...
        SEARCH1: {
            foreach my $d ( sort keys %$uData ) {
                ...
                while( my $this = shift @$data ) {
                    ...
                    isa_ok( $this->{'thetime'}, 'Time::Piece' );
                    isa_ok( $this->{'modified'}, 'Time::Piece' );
                }
            }
        }
    ...
    DATA_CLASS: {
        ...
        $Class = 'Kanadzuchi::BdDR::DailyUpdates::Data';
        ...
        CONSTRUCTOR: {
            isa_ok( $Dobj->db, 'Kanadzuchi::BdDR::DailyUpdates::Table' );
            isa_ok( $Dobj->handle, 'DBI::db' );
            isa_ok( $Dobj->data, 'ARRAY' );
            isa_ok( $Dobj->subtotal, 'ARRAY' );
        ...
        QUAERIT: {
            ...
            foreach my $unit ( qw/d w m y/ ) {
                ...
                isa_ok( $iterator, 'Kanadzuchi::Iterator' );

                while( my $e = $iterator->next ) {

                    isa_ok( $e, 'HASH' );
                    isa_ok( $e->{'thetime'}, 'Time::Piece' );
                    isa_ok( $e->{'modified'}, 'Time::Piece' );
