# $Id: 022_mime-parser.t,v 1.3.2.1 2013/08/30 23:05:12 ak Exp $
...
EOH
    ...
    IRREGULAR_CASES: {
        FALSE: foreach my $f (
            ...
            $object->parseit( \$f );
            is( $object->count, 0, '->parseit('.$argv.')->count = '.0 );
            $object->flush;
        }
        ...
        NEGATIVE: foreach my $n ( @{ $Kanadzuchi::Test::NegativeValues } ) {
            ...
            $object->parseit( \$n );
            is( $object->count, 0, '->parseit('.$n.')->count = '.0 );
            $object->flush;
        }
