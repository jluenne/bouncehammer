# $Id: 116_bddr-bouncelogs.t,v 1.8.2.1 2013/08/30 23:05:12 ak Exp $
...
my $Class = 'Kanadzuchi::BdDR::BounceLogs::Table';
my $Klass = 'Kanadzuchi::BdDR::BounceLogs';
...
SKIP: {
    ...
    MASTERTABLE: {
        foreach my $_mt ( qw/addressers senderdomains destinations hostgroups providers reasons/ ) {
            ...
            isa_ok( $Mtable->{ $_mt }, 'Kanadzuchi::BdDR::BounceLogs::Masters::Table' );
        }
    }
    ...
    CONSTRUCTOR: {
        ...
        isa_ok( $Btable->fields, 'HASH' );
        isa_ok( $Btable->fields->{'join'}, 'ARRAY' );
        isa_ok( $Btable->fields->{'trxn'}, 'ARRAY' );
        isa_ok( $Btable->fields->{'desc'}, 'ARRAY' );
    }
    ...
    PREPARE: {
        ...
        isa_ok( $JSON, 'ARRAY', 'Load file = '.$File );
    ...
    EACH_METHODS: {
        ...
        foreach my $_e ( @$Data ) {
            ...
            SEARCH: {
                ...
                isa_ok( $entity, 'ARRAY', '->search' );

                $entity = shift @{ $Btable->search( { 'token' => $_e->{'token'} }, $Page ) };
                isa_ok( $entity, 'HASH', '->search('.$_e->{'token'}.')' );
