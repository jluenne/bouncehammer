# $Id: 505_bin-summarizer.t,v 1.18.2.1 2013/08/30 23:05:13 ak Exp $
...
SKIP: {
    ...
    my $Test = new Kanadzuchi::Test::CLI(
            'command' => -x './dist/bin/summarizer' ? './dist/bin/summarizer' : './src/bin/summarizer.PL',
    ...
    EXEC: {
        ...
        ERROR_MESSAGES: {
            ...
            NO_AGGREGATION_OPTION: {
                ...
                like( $xresult->[4]->[0], qr/Aggregation option/, 'Aggregation option' );
            }

            INVALID_COLUMN_NAME: {
                $command = $Test->perl.$Test->command.$Opts.' -ax';
                $xresult = [ IPC::Cmd::run( 'command' => $command ) ];
                like( $xresult->[4]->[0], qr/Invalid column name/, 'Invalid column name' );
            }
        }
