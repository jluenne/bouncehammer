# $Id: 503_bin-databasectl.t,v 1.16.2.2 2013/08/30 23:05:12 ak Exp $
...
SKIP: {
    ...
    my $Test = new Kanadzuchi::Test::CLI(
            'command' => -x './dist/bin/databasectl' ? './dist/bin/databasectl' : './src/bin/databasectl.PL',
    ...
    EXEC_COMMAND: {
        ...
        NON_EXISTENT_LOG_DATE: {
            ...
            DATE_OPTIONS: foreach my $d ( 
                '--today', '--yesterday', '--before 2', '--date 1994-04-29', '--date 2038-02-01'  ) {

                $command = $Test->perl.$Test->command.$Opts.' --update '.$d;
                $xresult = [ IPC::Cmd::run( 'command' => $command ) ];
                like( $xresult->[4]->[0], qr/No regular log file to update/, 'No log file of the option '.$d );
            }
        }

        UPDATE_FROM_CONSOLE: {
            $command = $Test->perl.$Test->command.$Opts.' --update '.$Test->output;
            $xstatus = scalar(IPC::Cmd::run( 'command' => $command ));
            ok( $xstatus, 'UPDATE: '.$command );
        }
        ...
        SELECT_AND_VERIFY: {
            foreach my $y ( @$Yaml ) {
                ...
                if( $y->{'senderdomain'} ne 'example.org' ) {
                    ...
                    foreach my $_key ( 
                        qw/addresser recipient senderdomain destination token hostgroup reason provider/ ){
        ...
        UPDATE_FROM_CONSOLE_WITH_BATCHMODE1: {
            $command = $Test->perl.$Test->command.$Opts.' --batch --update '.$Test->output;
            $xresult = qx|$command|;
        ...
        UPDATE_FROM_CONSOLE_WITH_BATCHMODE3: {
            ...
            ok( $thisent->{'update'} > 15, '(3) status->update = '.$thisent->{'update'} );

            $thisent = $yamlobj->{'status'}->{'skipped'};
            is( $thisent->{'no-senderdomain'}, 1, '(3) skipped->no-senderdomain = 1' );
            ok( $thisent->{'too-old-or-same'} > 15 , '(3) skipped->too-old-or-same = '.$thisent->{'too-old-or-same'} );
            is( $thisent->{'is-whitelisted'}, 0, '(3) skipped->is-whitelisted = 0' );
            is( $thisent->{'exceeds-limit'}, 0, '(3) skipped->exceeds-limit = 0' );

            $thisent = $yamlobj->{'status'}->{'cache'}->{'positive'};
            ok( $thisent->{'bouncelogs'} > 50, '(3) cache->bouncelogs = '.$thisent->{'bouncelogs'} );
        ...
        UPDATE_FROM_CONSOLE_WITH_BATCHMODE5: {
            $command = $Test->perl.$Test->command.$Opts.' --batch --force --update '.$Test->output;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
        ...
        DISABLE_AND_DELETE_FROM_CONSOLE_WITH_BATCHMODE1: {
            ...
            # Disable
            $command = $Test->perl.$Test->command.$Opts.' --batch --disable --id '.$idnumis;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
            ...
            # Remove
            $command = $Test->perl.$Test->command.$Opts.' --batch --remove --id '.$idnumis;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
        ...
        DISABLE_AND_DELETE_FROM_CONSOLE_WITH_BATCHMODE2: {
            ...
            $command = $Test->perl.$Test->command.$Opts.' --batch --disable --token '.$tokenis;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
            ...
            # Remove
            $command = $Test->perl.$Test->command.$Opts.' --batch --remove --token '.$tokenis;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
        ...
        DISABLE_AND_DELETE_FROM_CONSOLE_WITH_BATCHMODE3: {
            ...
            # Disable
            $command = $Test->perl.$Test->command.$Opts.' --batch --disable --token '.$tokenis.' --id '.$idnumis;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
            ...
            # Remove
            $command = $Test->perl.$Test->command.$Opts.' --batch --remove --token '.$tokenis.' --id '.$idnumis;
            $xresult = qx|$command|;
            $yamlobj = JSON::Syck::Load( $xresult );
