# $Id: 012_time.t,v 1.8.2.1 2013/08/30 23:05:12 ak Exp $
...
my $T = new Kanadzuchi::Test(
    'class' => 'Kanadzuchi::Time',
    'methods' => [ 
        'to_second', 'monthname', 'dayofweek', 'hourname', 'o2d', 
...
METHODS: {
    ...
    TO_SECOND: {
        is( $class->to_second( '1d' ), 86400, $class.' 1 Day' );
        is( $class->to_second( '2w' ), ( 86400 * 7 * 2 ), $class.' 2 Weeks' );
        is( $class->to_second( '3f' ), ( 86400 * 14 * 3 ), $class.' 3 Fortnites' );
        is( int($class->to_second( '4l' )), 10205771, $class.' 4 Lunar months' );
        is( int($class->to_second( '5q' )), 39446190, $class.' 5 Quarters' );
        is( int($class->to_second( '6y' )), 189341712, $class.' 6 Years' );
        is( int($class->to_second( '7o' )), 883594656, $class.' 7 Olympiads' );

        is( int($class->to_second( 'gs' )), 23, $class.' 23.14(e^p) seconds' );
        is( int($class->to_second( 'pm' )), 188, $class.' 3.14(PI) minutes' );
        is( int($class->to_second( 'eh' )), 9785, $class.' 2.718(e) hours' );

        is( $class->to_second(-1), 0, 'The value: -1' );
        is( $class->to_second( -4294967296 ), 0, ' The value: -4294967296' );
    }
    ...
    MONTH_NAME: {
        ...
        isa_ok( $month, 'ARRAY', $class.'->monthname(0)' );
        is( $month->[0], 'Jan', $class.'->monthname(0)->[0]' );
        is( $month->[9], 'Oct', $class.'->monthname(0)->[9]' );

        $month = $class->monthname(1);
        isa_ok( $month, 'ARRAY', $class.'->monthname(1)' );
        is( $month->[1], 'February', $class.'->monthname(1)->[1]' );
        is( $month->[8], 'September', $class.'->monthname(1)->[8]' );
    }
    ...
    DAY_OF_WEEK: {
        ...
        isa_ok( $dayofweek, 'ARRAY', $class.'->dayofweek(0)' );
        is( $dayofweek->[1], 'Mon', $class.'->dayofweek(0)->[1]' );
        is( $dayofweek->[5], 'Fri', $class.'->dayofweek(0)->[5]' );

        $dayofweek = $class->dayofweek(1);
        isa_ok( $dayofweek, 'ARRAY', $class.'->dayofweek(1)' );
        is( $dayofweek->[0], 'Sunday', $class.'->dayofweek(1)->[0]' );
        is( $dayofweek->[6], 'Saturday', $class.'->dayofweek(1)->[6]' );
    }

    HOURS: {
        my $hours = $class->hourname(0);

        isa_ok( $hours, 'ARRAY', $class.'->hourname(0)' );
        is( $hours->[0], 'Midnight', $class.'->hourname(0)->[0]' );
        is( $hours->[6], 'Morning', $class.'->hourname(0)->[6]' );
        is( $hours->[12], 'Noon', $class.'->hourname(0)->[12]' );
        is( $hours->[18], 'Evening', $class.'->hourname(0)->[18]' );
    }
    ...
    TIMEZONE_TO_SECOND: {
        is( $class->tz2second('+0000'), 0, $class.'->tz2second(+0000)' );
        is( $class->tz2second('-0000'), 0, $class.'->tz2second(-0000)' );
        is( $class->tz2second('-0900'), -32400, $class.'->tz2second(-0900)' );
        is( $class->tz2second('+0900'), 32400, $class.'->tz2second(+0900)' );
        is( $class->tz2second('-1200'), -43200, $class.'->tz2second(-1200)' );
        is( $class->tz2second('+1200'), 43200, $class.'->tz2second(+1200)' );
        is( $class->tz2second('-1800'), undef, $class.'->tz2second(-1800)' );
        is( $class->tz2second('+1800'), undef, $class.'->tz2second(+1800)' );
        is( $class->tz2second('NULL'), undef, $class.'->tz2second(NULL)' );
        is( $class->tz2second, undef, $class.'->tz2second' );
    }

    SECOND_TO_TIMEZONE: {
        is( $class->second2tz(0), '+0000', $class.'->second2tz(0)' );
        is( $class->second2tz(-32400), '-0900', $class.'->second2tz(-32400)' );
        is( $class->second2tz(32400), '+0900', $class.'->second2tz(32400)' );
        is( $class->second2tz(-43200), '-1200', $class.'->second2tz(-43200)' );
        is( $class->second2tz(43200), '+1200', $class.'->second2tz(43200)' );
        is( $class->second2tz(-65535), '', $class.'->second2tz(65535)' );
        is( $class->second2tz(65535), '', $class.'->second2tz(65535)' );
        is( $class->second2tz(0e0), '+0000', $class.'->second2tz(0e0)' );
        is( $class->second2tz(), '+0000', $class.'->second2tz()' );
    }
