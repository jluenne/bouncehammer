# $Id: Kanadzuchi.pm,v 1.33.2.29 2014/02/23 12:49:46 ak Exp $
...
# Copyright (C) 2009-2014 Cubicroot Co. Ltd.
...
sub load {
    ...
    if( $conf ne '/dev/null' ) {
        ...
        try {
            ...
            if( not -e $conf ) {
                $et = $conf.': Config file does not exist, errno = '.Errno::ENOENT;
                Kanadzuchi::Exception::IO->throw( '-text' => $et );
            }

            if( $conf =~ m{[\x00-\x1f\x7f]} ) {
                $et = $conf.': Invalid config file name, errno = '.Errno::EINVAL;
                Kanadzuchi::Exception::File->throw( '-text' => $et );
            }

            if( not -T $conf ) {
                $et = $conf.': is not a text file, errno = '.Errno::EFTYPE;
                Kanadzuchi::Exception::File->throw( '-text' => $et );
            }

            if( not -r $conf ) {
                $et = $conf.': Cannot read, errno = '.Errno::EACCES;
            ...
            if( ref $self->{'config'} ne 'HASH' ) {
                $et = $conf.': is not YAML/JSON file, errno = '.Errno::EFTYPE;
                Kanadzuchi::Exception::File->throw( '-text' => $et );
            }

            if( ! defined $self->{'config'}->{'system'} && ! defined $self->{'config'}->{'version'} ) {

                $et = $conf.': It does not seem to config file of this system, errno = '.Errno::EFTYPE;
...
sub get_logfile {
    ...
    if( $char eq 'r' ) {
        ...
        $lopt->{'output'} =~ s{/\z}{}g;
        $file = sprintf("%s/%s.%s.%s", $lopt->{'output'}, $logf->{'prefix'}, $lopt->{'date'}, $logf->{'suffix'} );
