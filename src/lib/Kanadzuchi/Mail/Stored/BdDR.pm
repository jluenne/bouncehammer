# $Id: BdDR.pm,v 1.7.2.2 2013/08/30 05:55:25 ak Exp $
...
sub remove {
    ...
    $cond->{'id'} = $self->{'id'} if $self->{'id'};
    $cond->{'token'} = $self->{'token'} if $self->{'token'};

    if( $xtab->remove( $cond ) ) {

        $tbuf->purgeit( lc $xtab->alias, $self->{'token'} );
        return 1;
    }
    return 0;
...
sub disable {
    ...
    if( $xtab->disable( $cond ) ) {
        $xbuf = $tbuf->getit( lc $xtab->alias, $self->{'token'} );
        $xbuf->{'disabled'} = 1;
        $tbuf->setit( lc $xtab->alias, $self->{'token'}, $xbuf );
        return 1;
    }
    return 0;
}
