...
sub eatit {
    ...
    return Kanadzuchi::Iterator->new unless ref $mailx eq 'Kanadzuchi::Mbox';
    ...
    MIMEPARSER: while( my $_entity = shift @{ $mailx->messages } ) {
        ...
        unless( $bouncemesg->{'diagnosticcode'} ) {
            ...
            if( $xsmtpcharset && $xsmtpdiagnos ) {
                ...
                $tempheader->{'diagnosticcode'} = $mimeparser->getit('Diagnostic-Code')
                                                    || $mimeparser->getit('X-SMTP-Diagnosis')
                                                    || q();
            }
        ...
        next if( $@ && ref( $tempstring ) ne 'Time::Piece' );
...
sub is_userunknown {
    ...
    if( defined $self->{'reason'} && length $self->{'reason'} ) {
        ...
        my $rclass = 'Kanadzuchi::Mail::Why::RelayingDenied';
        my $nclass = 'Kanadzuchi::Mail::Why::NotAccept';
...
sub is_hostunknown {
    ...
    if( defined $self->{'reason'} && length $self->{'reason'} ) {
        ...
        if( $subj eq Kanadzuchi::RFC3463->causa( $stat ) ) {
            ...
            my $hclass = 'Kanadzuchi::Mail::Why::HostUnknown';
            my $dicode = $self->{'diagnosticcode'};
...
sub is_mailboxfull {
    ...
    if( defined $self->{'reason'} && length $self->{'reason'} ) {
        ...
        if( $subj eq Kanadzuchi::RFC3463->causa( $stat ) ) {
            ...
            my $mclass = 'Kanadzuchi::Mail::Why::MailboxFull';
            my $dicode = $self->{'diagnosticcode'};
...
sub is_exceedlimit {
    ...
    if( defined $self->{'reason'} && length $self->{'reason'} ) {
        ...
        if( $subj eq Kanadzuchi::RFC3463->causa( $stat ) ) {
            ...
            my $bclass = 'Kanadzuchi::Mail::Why::ExceedLimit';
            my $dicode = $self->{'diagnosticcode'};
...
sub is_toobigmesg {
    ...
    if( defined $self->{'reason'} && length $self->{'reason'} ) {
        ...
        if( $subj eq Kanadzuchi::RFC3463->causa( $stat ) ) {
            ...
            my $bclass = 'Kanadzuchi::Mail::Why::MesgTooBig';
            my $dicode = $self->{'diagnosticcode'};
