# $Id: Parser.pm,v 1.5.2.2 2013/08/30 05:55:35 ak Exp $
...
sub parseit {
    ...
    my $data = ref( $text ) eq 'SCALAR' ? $$text : $text;
    ...
    foreach my $thisline ( split( qq{\n}, $data ) ) {
        ...
        if( $thisline =~ m{\A(.+?)[:](.+)\z} ) {
            ...
            $self->{'data'}->{$headname} = [] 
                unless ref( $self->{'data'}->{ $headname } ) eq 'ARRAY';
...
sub getit {
    ...
    return q() unless ref( $self->{'data'}->{ $head } ) eq 'ARRAY';
