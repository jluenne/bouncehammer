# $Id: Masters.pm,v 1.13.2.2 2013/08/29 11:02:53 ak Exp $
...
sub whichtable {
    ...
    return $akmap->{ $first } if exists $akmap->{ $first };
    return q();
}
...
sub insert {
    ...
    eval {
        ...
        my $_that = $_tobj->find_or_insert( $self->{'table'}, $_data );
...
sub update {
    ...
    return 0 if( ref( $data ) ne 'HASH' || ref( $cond ) ne 'HASH' );
