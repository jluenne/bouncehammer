# $Id: BounceLogs.pm,v 1.14.2.2 2013/08/29 11:02:53 ak Exp $
...
install_table( 't_bouncelogs' => schema { 
    pk('id');
    columns( 
        qw/ id addresser recipient senderdomain destination
...
sub groupby {
    ...
    return [] unless( grep { $name eq $_ } ( @{ $self->{'fields'}->{'join'} }, 'hostgroup', 'reason' ) );
    ...
    eval {
        if( $name eq 'hostgroup' || $name eq 'reason' ) {
            ...
            my $_ropt = { 
                'group' => { 
                    'column' => $_mtab->table.'.'.$_mtab->field
                },
                'select' => [ 
                    'COUNT(token) AS x', 
                    'SUM(frequency) AS y', 
