# $Id: DailyUpdates.pm,v 1.4.2.4 2013/08/29 11:02:53 ak Exp $
...
install_table( 't_dailyupdates' => schema { 
    pk('id');
    columns( 
        qw/id thetime thedate inserted updated skipped
...
sub new {
    ...
    my $class = shift;
    my $argvs = { @_ };
    my $klass = 'Kanadzuchi::BdDR::DailyUpdates';
...
sub new {
    ...
    map {
        $argvs->{ $_ } = [] if( ! defined $argvs->{ $_ } || ref( $argvs->{ $_ } ) ne 'ARRAY' )
    } ( 'data', 'subtotal' );
