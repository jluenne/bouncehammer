# $Id: CLI.pm,v 1.19.2.6 2013/08/30 05:54:48 ak Exp $
...
sub init {
    ...
    try {
        ...
        #           |__/                                   
        if( ! defined $bcli || ref( $bcli ) ne 'Kanadzuchi' ) {

            $_error = 'No system object, errno = '.Errno::EINVAL;
        ...
        if( ! defined $_kconf->{'system'} && ! defined $_kconf->{'version'} ) {
            ...
            $_error = 'Config file is not loaded, errno = '.Errno::EINVAL;
            Kanadzuchi::Exception::System->throw( '-text' => $_error );
        }
        ...
        if( -e $self->{'pf'} ) {
            $_error = $self->{'pf'}.': pid file exists, errno = '.Errno::EEXIST;
            Kanadzuchi::Exception::System->throw( '-text' => $_error );
        }
    ...
    $self->d( 1, sprintf( "%s/%s %s\n", $bcli->myname, $self->{'calledfrom'}, $bcli->version ));
...
sub catch_signal {
    ...
    my $mesg = '***Catch the signal('.$sign.')';

    if( $sign eq 'ALRM' ){ 
        $mesg = 'Timed out, No data from STDIN';
    }
